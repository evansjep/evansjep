
export const siteConfig = {
  name: "Evans Jep",
  
  identity: {
    name: "Evans Jep",
    role: "Founder • Builder • Steward",
    mission: "Creating lasting value and advancing God's purpose.",
  },

  description:
    "Founder operating system documenting the building of technology, commerce, and identity systems across Africa and global digital infrastructure. Focused on long-term value creation, execution, and institutional building.",

  focus:
    "Building enduring systems, companies, and platforms that outlast their builder.",

  ecosystem: [
    {
      name: "Clesla",
      type: "Commerce Infrastructure",
      status: "Building",
      description: "Audio systems commerce and operations infrastructure.",
    },
    {
      name: "Dee Cleaneagles",
      type: "Technology Commerce Platform",
      status: "Building",
      description:
        "Cross-border technology commerce platform focused on Africa.",
    },
    {
      name: "Orviansla",
      type: "Creator Infrastructure",
      status: "Design Phase",
      description:
        "Identity and commerce infrastructure for creators and businesses.",
    },
  ],

  principles: [
    "Long-term thinking over short-term output",
    "Systems over standalone products",
    "Execution over perception",
    "Clarity over complexity",
    "Stewardship over ownership",
  ],

  links: {
    x: "https://x.com/RealEvansJep",
    linkedin: "https://linkedin.com/in/realevansjep",
    github: "https://github.com/RealEvansJep",
    website: "https://evansjep.com",
  },
};
